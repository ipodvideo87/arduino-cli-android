...
- Use the `arduino-cli acl transport list|diagnose|acquire|probe-fd|stream-status`
  commands as the safe diagnostic surface for Termux USB work. Do not claim
  Android USB success unless it has been validated on native Termux.
...
- Treat the upload engine foundation as a transport-neutral prepare-only planner/executor stack. It may validate firmware packages, derive ordered upload operations, and surface a canonical executor report, but it must not open real transport streams or send bytes until the transport execution milestone is explicitly validated.
- Treat `arduino-cli acl workflow upload <firmware-package>` as a positional prepare-only workflow unless the CLI contract is explicitly changed. Do not add `--dry-run` or `--package` flags without updating the architecture docs and validation findings.
